...
const seedMasterData = async () => {
  try {
    ...
    logger.info("Seeding Items (with Unsplash Images)...");
    ...
    await ItemModel.insertMany([
      ...
      },
      {
        ...
        isAvailable: false, // Sedang kosong misalnya
        modifier: [],
        addon: [],
      },
    ]);

    logger.info("Successfully seeded Master Data!");
    process.exit(0);
  } catch (error) {
    logger.error(error, "Failed to seed Master Data");
    process.exit(1);
  }
};
